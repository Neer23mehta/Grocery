  ...
  return (
    <div className='mt-2.5'>
      {/* <Draggable id={branch.id}> */}
        <div className='flex flex-row justify-between border outline-none rounded-md px-2.5 py-2 accordion-item active tree'>
          <span className='mr-2 px-2 py-0.5 border-2 rounded-md border-y' onClick={() => { if (depth < Depth) { edit(branch.id, branch.value) } }}>{branch.value}</span>
          <div className="flex space-x-2">
            {depth < Depth && (
              <>
                <strong onClick={() => setInputOpen(true)} className='text-2xl border-2 text-green-700 px-2 rounded-md cursor-pointer'>+</strong>
                <span onClick={() => edit(branch.id, branch.value)} className='text-xl text-blue-800 border-2 px-1 py-1.5 rounded-md cursor-pointer'><MdEdit /></span>
              </>
            )}
            <span onClick={() => del(branch.id)} className='text-xl text-red-800 border-2 px-1 py-1.5 rounded-md cursor-pointer'><RiDeleteBin5Fill /></span>
          </div>
        </div>
      {/* </Draggable> */}
          ...
          <form onSubmit={formik.handleSubmit}>
            <DialogContent>
              ...
              {formik.touched.inputValue && formik.errors.inputValue && (
                <div className="text-red-600 text-sm mt-1">{formik.errors.inputValue}</div>
              )}
            </DialogContent>
      ...
      <DndContext onDragEnd={handleDragEnd}>
        <div className='flex justify-end mr-5 mt-5'>
          <Droppable id="droppable">
            <span className='px-5 py-3 cursor-pointer border text-xl rounded-md font-bold bg-red-600 text-white text-center' onClick={() => setDatas([])}>Delete</span>
          </Droppable>
        </div>
        ...
        <div className="py-5 flex flex-col w-full justify-center items-center">
          <div className='flex space-x-2 mb-4 flex-row'>
            <div className='flex flex-row border rounded-md'>
              <div className='flex flex-row items-center'>
                <button className='ml-3'><IoSearchSharp size={30} /></button>
                <input
                  value={search}
          ...
          <div className='flex justify-center items-start space-x-5 text-xl px-5 py-2 w-full'>
            {datas
              ?.filter(item => search === "" || item.value.toLowerCase().includes(search.toLowerCase()))
              .map(item => (
                <Tree
                  key={item.id}
                  branch={item}
