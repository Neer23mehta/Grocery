    ...
    <div className='p-5'>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filtertedMObile.map((phone) => (
          <div key={phone.id} className="bg-white shadow-lg rounded-lg overflow-hidden transform transition-transform duration-300 hover:scale-105">
            <img src={phone.image} alt={phone.name} className="w-full h-56 mt-5 object-contain" />
            <div className="p-4">
              <h2 className="text-xl font-semibold mb-2 text-gray-800">{phone.name}</h2>
              ...
              <button
                ...
                className="mt-5 w-full text-white py-2 rounded bg-gradient-to-r from-yellow-400 to-orange-600 transition"
              >
                Add to Cart
              </button>
            </div>
