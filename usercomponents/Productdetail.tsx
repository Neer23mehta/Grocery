...
const Productdetail = () => {
  ...
  const fetchDataID = async () => {
    try {
      ...
      const res2 = await axios.get(`https://fakestoreapi.com/products/${id}`)
      setProduct(res.data);
      setData(res2.data)
    } catch (error) {
  ...
  if (!product || data) return <div className="text-center mt-10 text-lg">Loading...</div>;
      ...
      {/* <div className="bg-white rounded-lg shadow-lg overflow-hidden grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="p-6 flex flex-col justify-between">
          <div>
            ...
            <p className="text-gray-600 text-base mb-6 leading-relaxed">{data.description}</p>
          </div>
          <div>
            <span className="text-2xl font-semibold text-green-600">Price:${data.price}</span>
          </div>
        </div>
      </div> */}
    </div>
