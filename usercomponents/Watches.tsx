...
const Watch = () => {
  ...
  const filteredWatch = watch.filter((item) => item.brand.toLowerCase().includes(search.toLowerCase()) || item.name.toLowerCase().includes(search.toLowerCase()))
  const handleAddToCart = (bags: Watch) => {
    dispatch(
      addProduct({
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">Watches</h1>
        ...
        {filteredWatch.map((phone) => (
          <div key={phone.id} className="bg-white shadow-md rounded-lg overflow-hidden">
            ...
            <div className="p-4">
              ...
              <p className="text-gray-600"><span className="font-medium">Strap:</span> {phone.strap}</p>
